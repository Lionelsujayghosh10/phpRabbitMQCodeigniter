...
class ClassSection extends CI_Controller {
	...
	public function createClass(){
		try{
			if(!empty($this->input->post()) && $this->input->post('Create') === 'Create') {
				$conditionArray   =  array('class_code' => trim(strip_tags($this->input->post('class_code'))), 'isDelete' => '0');
				$checkAlreadyGiven = $this->QueryModel->getWhere($conditionArray, 'classes');
				if(empty($checkAlreadyGiven)){
					...
					$this->QueryModel->insertDataIntoTable($createClass, 'classes');
					$this->session->set_flashdata('success', 'Class created successfully.');
					redirect('ClassSection/createClass', 'refresh');
				}else{
					$this->session->set_flashdata('error', 'Class Code already exists.');
					redirect('ClassSection/createClass', 'refresh');
				}
			}else{
				$this->load->view('createclass.php');
			}
		}catch(Exception $e){
			echo $e->getMessage(); die;
		}
